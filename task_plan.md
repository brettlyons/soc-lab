...
|-------|---------|------------|
| **RDP as domain users (LAB\mscott etc.) fails from aurora host** — **RESOLVED** | 5 | Root cause: domain users not in local **Remote Desktop Users** group on workstations. Domain join does not auto-grant RDP — local admins get it free, domain users don't. Fix: RDP in as labadmin → `Add-LocalGroupMember -Group "Remote Desktop Users" -Member "LAB\Domain Users"`. Red herrings along the way: (1) Kerberos "Cannot find KDC" error → fixed by `/etc/krb5.conf.d/lab-local.conf` on aurora (still a valid fix — KDC config was missing); (2) "Connection reset by peer" after NLA completes = authorization rejection, not auth failure. Add OpenSSH to Autounattend templates to avoid needing RDP for future one-liners. |
...
| win-forensic won't start: "Could not access KVM kernel module: No such file or directory" | 1 | Root cause: `/dev/kvm` owned by `systemd-network:0660` instead of `kvm:0666`. libvirt creates a private devns for QEMU — when bind-mounting /dev/kvm into it, the wrong group/mode causes ENOENT inside the namespace. Appears to be a one-time boot race (systemd-network grabbed the node before udev finished). Distro rule `/usr/lib/udev/rules.d/50-udev-default.rules` already sets `GROUP="kvm", MODE="0666"` — no custom rule needed. If it recurs: check `stat /dev/kvm` and compare Gid against `kvm`. Quick fix: `sudo chown root:kvm /dev/kvm && sudo chmod 0666 /dev/kvm`. If persistent across reboots, investigate with `udevadm monitor` during boot. |
| **aurora → lefthand lab-net forwarding blocked** — **IN PROGRESS** | 3 | Goal: reach lefthand VMs (192.168.10.0/24) from aurora (172.16.1.147). Root complication: both aurora and lefthand use 192.168.10.0/24, so aurora's existing route via its own fw-router (192.168.122.10) conflicts. Fix attempt 1: add firewalld policy aurora-to-lab (ingress FedoraServer → egress libvirt, target ACCEPT) — policy created and active but packets still rejected with ICMP "Destination Port Unreachable" from 172.16.1.114. Fix attempt 2: add 172.16.1.147 to lefthand trusted zone — same result. Fix attempt 3: replaced aurora's /24 route with 192.168.10.0/24 via 172.16.1.114 + specific host route 192.168.10.60 via 192.168.122.10 for kali — routing correct but lefthand still sends back ICMP port-unreachable. From lefthand itself, 192.168.10.10 is pingable (via 192.168.122.10 dev virbr0). Suspect: libvirt_network nftables chains (guest_input/guest_cross) may be restricting traffic arriving on enp0s31f6 rather than virbr*. Next: inspect `nft list chain ip libvirt_network guest_input` etc. |
