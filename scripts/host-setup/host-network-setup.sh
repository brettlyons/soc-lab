...
#   4. Enables VM autostart (fw-router, wazuh, splunk)
#   5. Configures systemd-resolved stub zone for *.lab.local -> fw-router dnsmasq
#
# Prerequisites:
...
#   - fw-router running dnsmasq (Phase 3c) for step 5 to verify correctly
#   - wazuh and splunk VMs defined
#
...
#    Note: 192.168.40.0/24 route is kept for Phase 10 (sandbox VM).
# --------------------------------------------------------------------------
echo "[2/5] Installing soc-lab-routes systemd unit..."
...
sudo ip route replace 192.168.40.0/24 via 192.168.122.10 dev virbr0 2>/dev/null || true
...
#    fw-router and wazuh: core infrastructure, always autostart
#    splunk: autostart since it's a persistent service VM
# --------------------------------------------------------------------------
...
sudo virsh --connect qemu:///system autostart splunk      && echo "      splunk:     autostart enabled"

# --------------------------------------------------------------------------
# 5. systemd-resolved stub zone for lab.local
#    Routes *.lab.local DNS queries to fw-router dnsmasq (192.168.10.1).
#    Requires dnsmasq to be running on fw-router (Phase 3c).
# --------------------------------------------------------------------------
echo "[5/5] Configuring systemd-resolved stub zone for lab.local..."
bash "$SCRIPT_DIR/lab-dns-stub.sh"
